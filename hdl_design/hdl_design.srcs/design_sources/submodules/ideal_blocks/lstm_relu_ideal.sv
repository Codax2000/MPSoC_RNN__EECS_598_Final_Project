...
ReLU activation function block with 4 inputs. Assumes a signed input and
computes max(0, data_i). Conforms to standard handshake signals.

Only computes ReLU(data_i[0]) - used for testing only
*/

module lstm_relu_ideal #(
    parameter N = 16
) (
    input logic signed [3:0][N-1:0] data_i,
    input logic valid_i,
    ...
    assign data_o_n = (ready_o && valid_i) ? data_i[0] : data_o_r;
