...
#(
    ...
    logic signed [WIDTH-1:0] lut [0:15];
    logic signed 

    initial begin
        lut_expand[0] = 16'b0000000101011010; //arctanh(1-2^-1-2)
        lut_expand[1] = 16'b0000000011111001;
    end

    initial begin
        lut[0] = 16'b0000000000000000;
        lut[1] = 16'b0000000000000000;
        lut[2] = 16'b0000000010001100; // arctanh(2^-1)
        lut[3] = 16'b0000000001000001;
        lut[4] = 16'b0000000000100000;
        lut[5] = 16'b0000000000010000;
        lut[6] = 16'b0000000000010000;
        lut[7] = 16'b0000000000001000;
        lut[8] = 16'b0000000000000100;
        lut[9] = 16'b0000000000000010;
        lut[10] = 16'b0000000000000001;
    ...
    always_comb begin
        if (i <= 2) begin
            if (z_in < 0) begin
                ...
                z_temp = z_in + lut_expand[0]; 

            end else begin    
                x_temp = x_in + (y_in - (y_in >>> (i + 2))); 
                y_temp = y_in + (x_in - (x_in >>> (i + 2)));
                z_temp = z_in - lut_expand[1];
            end
        end
        else begin
            if (z_in < 0) begin
